...
class NavigationIdentifierMapperTest {
    ...
    fun serializeNavigationIdentifier() {
        val nav = NavigationIdentifier.PostNavigationIdentifier("name", "avatar", LoginProfile("teamName", Client("id", "secret"), Token("accessToken", "tokenType", "scope", 0L)))
        ...
        assert(deserialize.loginProfile.teamName == "teamName")
    ...
    fun serializePostDetail() {
        val nav = NavigationIdentifier.PostDetailNavigationIdentifier("name", "avatar", LoginProfile("teamName", Client("id", "secret"), Token("accessToken", "tokenType", "scope", 0L)), "fullName")
        val json = NavigationIdentifierMapper().gson.toJson(nav, NavigationIdentifier::class.java)
        val deserialize = NavigationIdentifierMapper().gson.fromJson(json, NavigationIdentifier::class.java)
